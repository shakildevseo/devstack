...
const technologiesDataUrl = new URL('../data/technologies.json', import.meta.url).href;
...
export const Technology = () => {
    ...
    useEffect(() => {
        const loadTechnologies = async () => {
            try {
                const response = await fetch(technologiesDataUrl);
                ...
                setLoadError('Unable to load technologies right now.');
            } finally {
                setIsLoading(false);
            }
        };
            ...
            <section id="technologies" className="mx-auto max-w-7xl min-w-0 px-3 py-3 sm:px-8 sm:py-7 lg:px-10 lg:py-10">
                ...
                <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.7fr)_minmax(180px,0.6fr)]">
                    <section className="min-w-0" aria-labelledby="technology-list-title">
                        {isLoading ? (
                            <div className="flex min-h-64 items-center justify-center rounded-2xl border border-slate-200 bg-white text-sm font-semibold text-slate-500 shadow-sm" role="status" aria-live="polite">
                                <span className="mr-3 h-5 w-5 animate-spin rounded-full border-2 border-slate-200 border-t-orange-500" aria-hidden="true" />
                                Loading technologies...
                            </div>
                        ) : loadError ? (
                            <div className="flex min-h-64 items-center justify-center rounded-2xl border border-red-200 bg-red-50 px-4 text-center text-sm font-semibold text-red-600" role="alert">
                                {loadError}
                            </div>
                        ) : (
